...
function EnrollCard({ course }) {
  ...
  const handleEnroll = async () => {
    try {
      ...
      // 🔥 API CALL
      const res = await axios.post("http://localhost:5001/api/enrollments/enroll", {
        userId: user._id,
        courseId: course._id
      });
      ...
      if (msg === "Already enrolled") {
        // 🔥 if already enrolled → go directly
        navigate(`/learn/${course._id}`);
      } else {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-[2rem] shadow-xl overflow-hidden sticky top-24 w-full">
